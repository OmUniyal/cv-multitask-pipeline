...
> **Note:** These endpoints are available when running the FastAPI server locally or via Docker (`uvicorn` or `docker-compose up`). The hosted HuggingFace Spaces demo runs a standalone Gradio app without the FastAPI layer.
...
Full interactive API docs at `/docs` (Swagger UI auto-generated by FastAPI) — visit `http://localhost:8000/docs` after starting the server locally.
...
- **Closed-set classification** — the model always predicts one of 20 VOC classes, even for out-of-distribution images (e.g. abstract art, unrelated objects). It has no "none of the above" option. A production system would add an OOD detection layer or a confidence threshold below which predictions are rejected.
- **Overfitting after epoch 5** — validation performance plateaued early; the model would benefit from stronger regularization or a larger/more diverse training set.
- **Detection head simplicity** — a single regression head for one bbox per image cannot handle multiple objects per image or complex scenes. Dedicated detection architectures (YOLO, Faster R-CNN) address this with region proposals or anchor boxes.
